CREATE TYPE "public"."analysis_run_status" AS ENUM('accepted', 'queued', 'deferred', 'failed');--> statement-breakpoint
CREATE TYPE "public"."membership_role" AS ENUM('owner', 'investigator', 'viewer');--> statement-breakpoint
...
CREATE TABLE "analysis_runs" (
	...
	"status" "analysis_run_status" DEFAULT 'accepted' NOT NULL,
	"failure_code" text,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE "evidence_metadata" (
	...
	"content_type" text DEFAULT 'message/rfc822' NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "evidence_metadata_object_key_unique" UNIQUE("object_key")
);
--> statement-breakpoint
CREATE TABLE "memberships" (
	...
	"organization_id" text NOT NULL,
	"user_id" text NOT NULL,
	"role" "membership_role" DEFAULT 'investigator' NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
ALTER TABLE "evidence_metadata" ADD CONSTRAINT "evidence_metadata_organization_id_organizations_id_fk" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "evidence_metadata" ADD CONSTRAINT "evidence_metadata_case_id_cases_id_fk" FOREIGN KEY ("case_id") REFERENCES "public"."cases"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "analysis_runs_org_case_idx" ON "analysis_runs" USING btree ("organization_id","case_id");--> statement-breakpoint
CREATE INDEX "audit_records_organization_idx" ON "audit_records" USING btree ("organization_id");--> statement-breakpoint
CREATE INDEX "cases_organization_idx" ON "cases" USING btree ("organization_id");--> statement-breakpoint
CREATE INDEX "evidence_org_case_idx" ON "evidence_metadata" USING btree ("organization_id","case_id");--> statement-breakpoint
